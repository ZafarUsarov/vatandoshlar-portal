...
export type GuideCreateActionState = {
  error: string | null;
};
...
export async function createGuideArticleAction(
  _previousState: GuideCreateActionState,
  formData: FormData,
): Promise<GuideCreateActionState> {
  const locale =
    await getLocale();
  ...
  try {
    ...
    console.error(
      "Failed to create Guide article:",
      error,
    );
