...
export type JobCreateActionState = {
  error: string | null;
};
...
export async function createJobAction(
  _previousState: JobCreateActionState,
  formData: FormData,
): Promise<JobCreateActionState> {
  ...
  const shortTitleUz =
    getString(formData, "shortTitleUz");

  const shortTitleDe =
    getString(formData, "shortTitleDe");

  const descriptionUz =
    getString(formData, "descriptionUz");

  const descriptionDe =
    getString(formData, "descriptionDe");

  const categoryValue =
    getString(formData, "category");

  const icon =
    getString(formData, "icon");

  const audienceUz =
    getString(formData, "audienceUz");

  const audienceDe =
    getString(formData, "audienceDe");
  ...
  if (
    highlightsUz.length !== highlightsDe.length ||
    stepsUz.length !== stepsDe.length ||
    importantNotesUz.length !== importantNotesDe.length
  ) {
    return {
      error:
        appLocale === "de"
  ...
  if (!isValidUrl(officialSourceUrl)) {
    return {
      error:
        appLocale === "de"
  ...
  try {
    ...
    console.error(
      "Failed to create admin job guide:",
      error,
    );
