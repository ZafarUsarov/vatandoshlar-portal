...
import {
  updateAdminSupportContribution,
...
export type SupportEditActionState = {
  error: string | null;
};
...
export async function updateSupportContributionAction(
  contributionId: string,
  _previousState: SupportEditActionState,
  formData: FormData,
): Promise<SupportEditActionState> {
  ...
  if (!/^\d+$/.test(contributionId)) {
    return {
      error:
        appLocale === "de"
          ? "Ungültige Beitrags-ID."
          : "Contribution ID noto‘g‘ri.",
    };
  }
  ...
  try {
    const updated =
      await updateAdminSupportContribution(
        contributionId,
        {
          supporterName:
      ...
      return {
        error:
          appLocale === "de"
            ...
            : "Contribution topilmadi.",
      };
    }
  } catch (error) {
    console.error(
      "Failed to update support contribution:",
      error,
    );
