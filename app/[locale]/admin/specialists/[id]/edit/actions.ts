...
export type SpecialistEditActionState = {
  error: string | null;
};
...
function getCategories(
  ...
  return formData
    ...
    .filter(
      (
        value,
      ): value is AdminSpecialistCategory =>
        categoryValues.includes(
          value as AdminSpecialistCategory,
        ),
    );
}
...
function getLanguages(
  ...
  return formData
    ...
    .filter(
      (
        value,
      ): value is AdminSpecialistLanguage =>
        languageValues.includes(
          value as AdminSpecialistLanguage,
        ),
    );
}
...
export async function updateSpecialistAction(
  _previousState: SpecialistEditActionState,
  formData: FormData,
): Promise<SpecialistEditActionState> {
  ...
  if (
    Boolean(serviceAreaUz) !==
    Boolean(serviceAreaDe)
  ) {
    return {
      error:
        appLocale === "de"
  ...
  if (
    Boolean(pricingNoteUz) !==
    Boolean(pricingNoteDe)
  ) {
    return {
      error:
        appLocale === "de"
  ...
  try {
    ...
    if (!updated) {
      return {
        error:
          appLocale === "de"
            ? "Die Fachkraft wurde nicht gefunden."
    ...
    console.error(
      "Failed to update admin specialist:",
      error,
    );
