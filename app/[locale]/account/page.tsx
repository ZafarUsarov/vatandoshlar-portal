  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-12 dark:bg-slate-950 sm:px-6">
      <div className="mx-auto max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        ...
        <p className="mt-3 max-w-xl text-sm leading-7 text-slate-600 dark:text-slate-400">{copy.description}</p>

        <dl className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-950/60">
          <dt className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{copy.email}</dt>
          <dd className="mt-2 break-all font-semibold text-slate-950 dark:text-white">{context.user.email}</dd>
        </dl>

        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <Link href="/account/profile" className="inline-flex min-h-11 items-center justify-center rounded-2xl bg-emerald-600 px-5 text-sm font-bold text-white transition hover:bg-emerald-500">
            {copy.profile}
          </Link>
          <Link href="/" className="inline-flex min-h-11 items-center justify-center rounded-2xl border border-slate-300 px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
            {copy.home}
          </Link>
        </div>
      </div>
