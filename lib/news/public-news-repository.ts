...
function getLocalizedContentType(
  ...
  const key =
    contentType in labels
      ? (contentType as keyof typeof labels)
      : "official_info";

  return labels[key][locale] as ContentType;
}
...
function toPublicNewsItem(
  ...
  return {
    ...
    location:
      locale === "de"
        ? row.location_de ?? undefined
        : row.location_uz ?? undefined,
...
export async function getPublishedNewsFromDatabase(
  locale: PublicNewsLocale,
): Promise<NewsItem[]> {
  if (!canUseDatabase()) {
    return [];
...
export async function getPublishedNewsBySlugFromDatabase(
  slug: string,
  locale: PublicNewsLocale,
  ...
  try {
    ...
    console.error(
      "Failed to load published news article from database:",
      error,
    );
